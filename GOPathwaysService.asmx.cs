...
namespace PathwaysService
{
	...
	///			<cvs_date>$Date: 2010/05/21 16:51:34 $</cvs_date>
	///			<cvs_header>$Header: /var/lib/cvs/PathCase_SystemsBiology/PathwaysService/GOPathwaysService.asmx.cs,v 1.2 2010/05/21 16:51:34 sarp Exp $</cvs_header>
	...
	public class GO2PathwaysService : System.Web.Services.WebService
	{
		...
		{
            //BE fixed (I think it's right)
            string variable =
                @"select GOName = go_terms.[Name], GOIDs.* from  
					(select distinct ECNumber=ec_go.ec_number, c.process_name, c.process_id, GOID=ec_go.GO_ID from 
							(select gpp1.*, process_name=processes.name from 
								(select * from 
								catalyzes 
								where process_id = @processID)gpp1 inner join processes on gpp1.process_id=processes.id 
						) c 
						inner join 
						ec_go 
						on c.ec_number = ec_go.ec_number 
					) GOIDs 
					inner join 
					go_terms
					on GOIDs.GOID = go_terms.[ID]";
